...
{
    mpu_write_reg(MPU6050_DEV_ADDR, PWR_MGMT_1, 0x00);	   //解除休眠状态
    mpu_write_reg(MPU6050_DEV_ADDR, SMPLRT_DIV, 0x07);      //125HZ采样率
    mpu_write_reg(MPU6050_DEV_ADDR, CONFIG, 0x04);          //
    mpu_write_reg(MPU6050_DEV_ADDR, GYRO_CONFIG, 0x18);     //250
    mpu_write_reg(MPU6050_DEV_ADDR, ACCEL_CONFIG, 0x10);    //8g
    mpu_write_reg(MPU6050_DEV_ADDR, User_Control, 0x00);
    mpu_write_reg(MPU6050_DEV_ADDR, INT_PIN_CFG, 0x02);
}
...
void Get_AccData(void)
{
    ...
    mpu_read_regs(MPU6050_DEV_ADDR, ACCEL_XOUT_H, dat, 6, M_IIC);  
...
void Get_Gyro(void)
{
    ...
    mpu_read_regs(MPU6050_DEV_ADDR, GYRO_XOUT_H, dat, 6, M_IIC);  
