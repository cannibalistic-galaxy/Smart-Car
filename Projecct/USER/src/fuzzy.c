...
#include "headfile.h"
int Pnn,Dnn;
float Fuzzy(float P,float D)    //输入偏差以及偏差变化量
{
/*输入量P语言值特征点*/ 
//float PFF[7]={-110,-90,-60,0,60,90,110};  //误差的模糊论域 //90，十字，180 //890
/*输入量D语言值特征点*/ 
//float DFF[5]={-36,-18,0,18,36};     //误差变化率EC的模糊论域 //直入90，直入十字
/*输出量U语言值特征点(根据赛道类型选择不同的输出值)*/
//float UFF[7]={0,15,20,35,50,80,120}; //输出的模糊论域 《=35% 补偿陀螺仪D

float PFF[7]={-6,-4,-2,0,2,4,6};  //误差的模糊论域
//输入量D语言值特征点
float DFF[5]={-9,-4,0,4,9};     //误差变化率EC的模糊论域
//输出量U语言值特征点(根据赛道类型选择不同的输出值)
//float UFF[7]={0,0.75,1,1.75,2.5,3,3.5}; //输出的模糊论域*/
float UFF[7]={0,0.75,1,1.5,2,2.5,3}; //输出的模糊论域*/
int rule[7][5]={            //重大模糊表
//  -2  -1   0   1   2 ec    e
  { 1,  1,  1,  0,  0}, //  -3
  { 4,  2,  1,  0,  0}, //  -2
